...
class Contributions(object):
    ...
    def _build_dict(self, C, FU_M_index, rev_dict, limit, limit_type):
        ...
        for fu_or_method, col in FU_M_index.items():
            ...
            cont_per = dict()
            cont_per.update({
                ('Total', ''): C[col, :].sum(), # TODO: Relative total? Doesnt work well with negative numbers
    ...
    def sort_array(data, limit=25, limit_type="number", total=None):
        ...
        if limit_type == "percent":
            ...
            limit = (np.abs(data) >= (total * limit)).sum() # Updated this compared to bw2. check: https://github.com/brightway-lca/brightway2-analyzer/issues/18
